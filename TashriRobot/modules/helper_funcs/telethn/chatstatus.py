...
from TashriRobot import DRAGONS
from TashriRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def Tashri_is_admin(chat_id: int):
    status = False
    Tashri = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Tashri.id == user.id:
            status = True
            break
    return status
